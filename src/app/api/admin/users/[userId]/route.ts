...
export async function DELETE(req: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  const target = await db.user.findUnique({ where: { id: userId }, select: { role: true, name: true } });
  if (!target) return NextResponse.json({ error: "User not found" }, { status: 404 });

  if ((ROLE_LEVEL[target.role] ?? 0) >= currentLevel) {
    return NextResponse.json({ error: "You cannot delete a user with equal or higher role." }, { status: 403 });
  }

  await db.user.delete({ where: { id: userId } });
  await logAudit(session.user.id, "USER_DELETE", "USER", userId, { name: target.name, role: target.role });
